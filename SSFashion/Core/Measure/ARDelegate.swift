...
class ARDelegate: NSObject, ARSCNViewDelegate, ObservableObject {
    ...
    func setARView(_ arView: ARSCNView) {
        ...
        configuration.planeDetection = [ARWorldTrackingConfiguration.PlaneDetection .horizontal, ARWorldTrackingConfiguration.PlaneDetection .vertical]
        arView.session.run(configuration)
        
       /* let configuration1 = ARWorldTrackingConfiguration()
        configuration1.planeDetection = .vertical
        arView.session.run(configuration1)*/
    ...
    private func raycastResult(fromLocation location: CGPoint) -> ARRaycastResult? {
        guard let arView = arView else { return nil }
            ...
            if let horizontalQuery = arView.raycastQuery(from: location, allowing: .existingPlaneGeometry, alignment: .horizontal) {
                query = horizontalQuery
            } else if let verticalQuery = arView.raycastQuery(from: location, allowing: .existingPlaneGeometry, alignment: .vertical) {
                query = verticalQuery
            } else {
                return nil
            }
